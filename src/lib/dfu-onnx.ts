...
// EfficientNet TF-style: [0,255] → x/127.5 - 1
function makeClfInputEff(imageData: ImageData) {
...
type Clf4 = {
  ...
  preprocessUsed: "EFFICIENTNET_TF";
  qualityScore: number;
...
export async function runClassifier(imageData: ImageData): Promise<Clf4> {
  ...
  const { p4, topIdx, pInfOnly, pInfPresent, pIscPresent } = await runClf4(session, makeClfInputEff(imageData));
...
export type InferOutput = {
  ...
  model: { sha256: string; nearConstant: boolean; preprocess: "EFFICIENTNET_TF"; quality: number };
};
...
function makeTensorFilledEff(val: number) {
  const H = CLF_INPUT_SIZE,
    W = CLF_INPUT_SIZE;
  const arr = new Float32Array(H * W * 3).fill(val);
  return new ort.Tensor("float32", arr, [1, H, W, 3]);
}
function makeTensorRandomEff(min = -1, max = 1) {
  const H = CLF_INPUT_SIZE,
    W = CLF_INPUT_SIZE;
...
async function validateClassifierSession(session: ort.InferenceSession, bytes?: ArrayBuffer) {
  ...
  const xs = [makeTensorFilledEff(-1), makeTensorFilledEff(1), makeTensorRandomEff(-1, 1)];
  const outs: number[][] = [];
  for (const x of xs) {
